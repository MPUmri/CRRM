% Simulation Maker.
% This script:
% - simulates concentration-time curves at 0.5s temporal resolution, using the Tofts Model
...
listCNR = simProp.CNR; % List of Contrast-Noise Ratios to simulate
nVox = simProp.nVox; % Number of replications for each CNR
listTRes = simProp.TRes; % List Temporal resolutions (in seconds)
...
disp('Simulation Maker started') % The script will start doing things now
...
for indCNR = 1:length(listCNR)
    ...
    % Create 'nVox' number of replications for each CNR
